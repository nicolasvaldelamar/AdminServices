...
export default function CotizacionForm() {
  ...
  const cargarDatos = async () => {
    try {
      const [clientesRes, serviciosRes] = await Promise.all([
        api.get('/clientes'),
        api.get('/servicios?estado=por_asignar,recibido,en_diagnostico')
      ])
  ...
  const cargarServicio = async (servicioId) => {
    try {
      ...
      setItems([{
        descripcion: `Servicio técnico - ${servicio.falla_reportada}`,
        cantidad: 1,
        precio_unitario: 0
      }])
    } catch (error) {
  ...
  const onSubmit = async (data) => {
    ...
    const cotizacionData = {
      ...
      items,
      ...totales,
      fecha_validez: data.fecha_validez || fechaValidez.toISOString().split('T')[0]
    }
    
    try {
      await api.post('/facturacion/cotizaciones', cotizacionData)
      navigate('/app/facturacion')
    } catch (error) {
      setError(error.response?.data?.error || 'Error al crear cotización')
    } finally {
      setLoading(false)
    }
  }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              >
                ...
                {servicios
                  .filter(s => s.cliente_id == clienteSeleccionado)
                  .map(servicio => (
            ...
            <div>
              ...
              <input
                ...
                className="input-field"
              />
              <p className="text-xs text-gray-500 mt-1">Por defecto: 30 días</p>
            </div>
          ...
          <div className="bg-purple-50 rounded-lg p-6">
            <div className="max-w-md ml-auto space-y-3">
              ...
              <div className="flex justify-between text-lg font-bold text-purple-900 pt-3 border-t-2 border-purple-200">
                <span>TOTAL:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <textarea
                ...
                defaultValue="Cotización válida por 30 días. Precios sujetos a cambio sin previo aviso."
              />
            </div>
          </div>
